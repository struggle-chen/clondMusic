...
Page({
   ...
   */
  data: {
    navDataList: [],   //初始化标签栏数据
    navId: '',            //初始化标签栏下面每个的id
    videoList: [],     //初始化视频数据
    videoId: '',        //视频的标识
    videoUpdateTime: [], // 记录video播放的时长
    triggered: false      //记录下拉状态
  },
  ...
  async handleNavData() {
    ...
    // console.log(navData);
    this.setData({
      navDataList: navData.data.data.slice(0, 14),
      navId: navData.data.data[0].id
    })
    // console.log(this.data.navId);
   
    this.handleVideoData(this.data.navId)
  ...
  // 处理视频数据
  async handleVideoData(navId) {
    let videoData = await request('/video/group', {id: navId})
    // console.log(navId); 
    
    let index = 0  
    videoData.data.datas.forEach(item => {
      item.index = index++
    });
    console.log(videoData);
    this.setData({
      videoList: videoData.data.datas
    })
  },
  // 视频播放时触发
  videoPlay(event) {
    ...
    * */
    ...
    // let vid = id
    // vid !== id && this.videoContext && this.videoContext.stop()
    // this.vid = id
    this.setData({
      videoId: id
    })
    // videoContext通过id跟一个video组件绑定操作对应的video组件。
  ...
  timeUpdate(event) {
    ...
    * */
    // console.log(event);
    // 来存储当前video的id和time
    let videoUpdateObj = {id: event.currentTarget.id, currentTime: event.detail.currentTime}
    // console.log(videoUpdateObj);
    let {videoUpdateTime} = this.data
    let videoItem = videoUpdateTime.find(item => item.id === event.currentTarget.id)
    if(videoItem) {
      videoItem.currentTime = event.detail.currentTime
    } else {
      videoUpdateTime.push(videoUpdateObj)
    } 
    this.setData({
      videoUpdateTime
    })
